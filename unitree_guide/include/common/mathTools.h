...
template <typename T> inline T saturation(const T a, Vec2 limits) {
  T lowLim, highLim;
 ...
 * @brief 
 * @tparam T 
 * @param[in] x             My Param doc
 * @param[in] windowRatio   My Param doc
 * @param[in] xRange        My Param doc
 * @param[in] yRange        My Param doc
 * @return T 
 */
template <typename T>
...
inline void updateCovariance(T1 &cov, T2 expPast, T3 newValue, double n) {
  ...
  if (fabs(n - 1) < 0.1) {
    ...
    cov = cov * (n - 1) / n + (newValue - expPast) *
                                  (newValue - expPast).transpose() * (n - 1) /
                                  (n * n);
  }
}
...
inline HomoMat homoMatrixInverse(HomoMat homoM) {
  ...
  homoInv.topRightCorner(3, 1) =
      -homoM.topLeftCorner(3, 3).transpose() * homoM.topRightCorner(3, 1);
  homoInv(3, 3) = 1;
  return homoInv;
}
...
class AvgCov {
public:
  AvgCov(unsigned int size, std::string name, bool avgOnly = false,
         unsigned int showPeriod = 1000, unsigned int waitCount = 5000,
         double zoomFactor = 10000)
      : _size(size), _showPeriod(showPeriod), _waitCount(waitCount),
        _zoomFactor(zoomFactor), _valueName(name), _avgOnly(avgOnly) {
    _exp.resize(size);
  ...
  void measure(VecX newValue) {
    ...
    if (_measureCount > _waitCount) {
      ...
      if (_measureCount % _showPeriod == 0) {
        std::cout << "******" << _valueName
                  << " measured count: " << _measureCount - _waitCount
                  << "******" << std::endl;
        std::cout << _zoomFactor << " Times Average of " << _valueName
                  << std::endl
                  << (_zoomFactor * _exp).transpose() << std::endl;
        if (!_avgOnly) {
          std::cout << _zoomFactor << " Times Covariance of " << _valueName
                    << std::endl
