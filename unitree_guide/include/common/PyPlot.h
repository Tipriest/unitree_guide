...
struct Plot {
  ...
  void printXY(std::string curveName, double xRough, int pointNum) {
    std::cout << "[DEBUG] Plot: " << plotName << ", Curve: " << curveName
              << std::endl;
...
public:
  ...
  void printXY(std::string plotName, std::string curveName, double xRough,
               int pointNum = 1);

  void addFrame(std::string plotName, double value);
  void addFrame(std::string plotName, double x, double value);

  template <typename T> void addFrame(std::string plotName, T *valueArray);
  template <typename T>
  ...
  template <typename T>
  void addFrame(std::string plotName, double x,
                const Eigen::MatrixBase<T> &vec);
...
inline PyPlot::PyPlot() { start = false; }
...
inline void PyPlot::printXY(std::string plotName, std::string curveName,
...
template <typename T>
inline void PyPlot::addFrame(std::string plotName,
                             const Eigen::MatrixBase<T> &vec) {
  _checkStart();
  Plot *plot = _getPlotPtr(plotName);
...
template <typename T>
inline void PyPlot::addFrame(std::string plotName, double x,
                             const Eigen::MatrixBase<T> &vec) {
  Plot *plot = _getPlotPtr(plotName);
