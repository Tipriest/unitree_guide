...
Vec12 QuadrupedRobot::getQd(const Vec34 &pos, const Vec34 &vel,
                            FrameType frame) {
  Vec12 qd;
  for (int i = 0; i < 4; ++i) {
...
// Forward Kinematics
Vec3 QuadrupedRobot::getFootPosition(LowlevelState &state, int id,
                                     FrameType frame) {
...
// Forward Kinematics
Vec34 QuadrupedRobot::getFeet2BPositions(LowlevelState &state,
                                         FrameType frame) {
  Vec34 feetPos;
...
Vec34 QuadrupedRobot::getFeet2BVelocities(LowlevelState &state,
                                          FrameType frame) {
  Vec34 feetVel;
  for (int i = 0; i < 4; ++i) {
...
A1Robot::A1Robot() {
  ...
  _feetPosNormalStand << 0.1805, 0.1805, -0.1805, -0.1805, -0.1308, 0.1308,
      -0.1308, 0.1308, -0.3180, -0.3180, -0.3180, -0.3180;
...
Go1Robot::Go1Robot() {
  ...
  _feetPosNormalStand << 0.1881, 0.1881, -0.1881, -0.1881, -0.1300, 0.1300,
