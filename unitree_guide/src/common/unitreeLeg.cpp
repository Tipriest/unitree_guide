...
// Forward Kinematics
/**
 * @brief 根据三个关节的角度q计算足端到基座(最初的根关节)坐标系原点的向量坐标
 * @param[in] q   三个关节的角度q
 * @return Vec3
 */
 ...
 * @return Vec3
 */
Vec3 QuadrupedLeg::calcPEe2B(Vec3 q) { return _pHip2B + calcPEe2H(q); }
 ...
 * @return Vec3
 */
Vec3 QuadrupedLeg::calcVEe(Vec3 q, Vec3 qd) { return calcJaco(q) * qd; }

// Inverse Kinematics
/**
 * @brief
 * 逆运动学，根据足端在机身坐标系或者在基座坐标系下的位置，求解三个关节的期望角度
 * @param[in] q             三个关节的角度q
 * @return Vec3
 */
...
float QuadrupedLeg::q2_ik(float q1, float q3, float px, float py, float pz,
                          float b3z, float b4z) {
