...
void Estimator::_initSystem() {
  ...
  _RInit << 0.008, 0.012, -0.000, -0.009, 0.012, 0.000, 0.009, -0.009, -0.000,
  ...
  /* A1 Worked */
  _Cu << 268.573, -43.819, -147.211, -43.819, 92.949, 58.082, -147.211, 58.082,
      302.120;
...
/* ROS odometry publisher */
  ...
  _pub = _nh.advertise<nav_msgs::Odometry>("legged_estimator_odom", 1);
...
void Estimator::run() {
  _feetH.setZero();
  _feetPosGlobalKine =
      _robModel->getFeet2BPositions(*_lowState, FrameType::GLOBAL);
  _feetVelGlobalKine =
      _robModel->getFeet2BVelocities(*_lowState, FrameType::GLOBAL);
...
Vec3 Estimator::getPosition() { return _xhat.segment(0, 3); }

Vec3 Estimator::getVelocity() { return _xhat.segment(3, 3); }

Vec3 Estimator::getFootPos(int i) {
  return getPosition() +
         _lowState->getRotMat() *
             _robModel->getFootPosition(*_lowState, i, FrameType::BODY);
}
