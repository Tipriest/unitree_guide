...
FeetEndCal::FeetEndCal(CtrlComponents *ctrlComp)
  ...
  for (int i = 0; i < 4; ++i) {
    _feetRadius(i) =
        sqrt(pow(feetPosBody(0, i), 2) + pow(feetPosBody(1, i), 2));
...
Vec3 FeetEndCal::calFootPos(int legID, Vec2 vxyGoalGlobal, float dYawGoal,
                            float phase) {
  ...
  _nextYaw = _dYaw * (1 - phase) * _Tswing + _dYaw * _Tstance / 2 +
             _kyaw * (dYawGoal - _dYaw);

  _nextStep(0) +=
      _feetRadius(legID) * cos(_yaw + _feetInitAngle(legID) + _nextYaw);
  _nextStep(1) +=
      _feetRadius(legID) * sin(_yaw + _feetInitAngle(legID) + _nextYaw);
