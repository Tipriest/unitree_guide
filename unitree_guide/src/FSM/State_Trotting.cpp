...
State_Trotting::~State_Trotting() { delete _gait; }
...
bool State_Trotting::checkStepOrNot() {
  if ((fabs(_vCmdBody(0)) > 0.03) || (fabs(_vCmdBody(1)) > 0.03) ||
      (fabs(_posError(0)) > 0.08) || (fabs(_posError(1)) > 0.08) ||
      (fabs(_velError(0)) > 0.05) || (fabs(_velError(1)) > 0.05) ||
      (fabs(_dYawCmd) > 0.20)) {
    return true;
  } else {
    return false;
...
void State_Trotting::calcTau() {
  ...
  _forceFeetGlobal =
      -_balCtrl->calF(_ddPcd, _dWbd, _B2G_RotMat, _posFeet2BGlobal, *_contact);
...
void State_Trotting::calcQQd() {
  ...
  for (int i = 0; i < 4; ++i) {
    _posFeet2BGoal.col(i) =
        _G2B_RotMat * (_posFeetGlobalGoal.col(i) - _posBody);
    _velFeet2BGoal.col(i) =
        _G2B_RotMat * (_velFeetGlobalGoal.col(i) - _velBody);
  ...
  _qdGoal = vec12ToVec34(
      _robModel->getQd(_posFeet2B, _velFeet2BGoal, FrameType::BODY));
}
