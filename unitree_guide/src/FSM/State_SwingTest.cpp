...
void State_SwingTest::enter() {
  ...
  _initFeetPos =
      _ctrlComp->robotModel->getFeet2BPositions(*_lowState, FrameType::HIP);
...
void State_SwingTest::run() {
  ...
  if (_userValue.ly > 0) {
    _posGoal(0) =
        invNormalize(_userValue.ly, _initPos(0), _initPos(0) + _xMax, 0, 1);
  } else {
    _posGoal(0) =
        invNormalize(_userValue.ly, _initPos(0) + _xMin, _initPos(0), -1, 0);
  }

  if (_userValue.lx > 0) {
    _posGoal(1) =
        invNormalize(_userValue.lx, _initPos(1, 0), _initPos(1) + _yMax, 0, 1);
  } else {
    _posGoal(1) =
        invNormalize(_userValue.lx, _initPos(1) + _yMin, _initPos(1), -1, 0);
  }

  if (_userValue.ry > 0) {
    _posGoal(2) =
        invNormalize(_userValue.ry, _initPos(2), _initPos(2) + _zMax, 0, 1);
  } else {
    _posGoal(2) =
        invNormalize(_userValue.ry, _initPos(2) + _zMin, _initPos(2), -1, 0);
  }
...
void State_SwingTest::exit() { _ctrlComp->ioInter->zeroCmdPanel(); }
...
void State_SwingTest::_torqueCtrl() {
  Vec3 pos0 =
      _ctrlComp->robotModel->getFootPosition(*_lowState, 0, FrameType::HIP);
